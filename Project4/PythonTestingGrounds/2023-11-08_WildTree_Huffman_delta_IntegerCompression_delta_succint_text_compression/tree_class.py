...
class NodeClass:
	...
	def write_details(idx_list):
		string_to_write  = chr(NodeClass.pop_count | 0x80)		# Merged pop marker '<' as MSbit, and pop count as 7 LSbits. {1'b<, 7'h7F}
		string_to_write += "".join(str(x) for x in NodeClass.scan_stack[len(NodeClass.scan_stack) - NodeClass.push_count:])
		NodeClass.file_ptr_text.write(string_to_write.encode())
		...
		NodeClass.file_ptr_indx.write(data)
