...
int treenode::addElement(std::string word, unsigned int idx) {
	...
	return this->subnode[key]->addElement(word.substr(1), idx);
}
...
void treenode::print_details(std::vector<unsigned int> &idx_list) {
	std::cout << scan_stack << ": ";
...
void treenode::save_details(std::vector<unsigned int> &idx_list) {
	unique_elem_stream.unique_elem.push_back(scan_stack);
	unique_elem_stream.idx_list_ptrs.push_back(&idx_list);
	return;
}

// format [1byte seqLength, N byte seq, 2 byte idx_list_length, 4 byte first idx, N byte compressed delta stream]
// Uses Huffman coding
void treenode::write_details_huffman(std::vector<unsigned int> &idx_list) {
	static unsigned len_var;
	...
	*((unsigned short *) &compressed_sequence[c_idx]) = (unsigned short) (len_var | 0xF000);
	...
	for(int i=0; i<len_var; i++) {
		// huffman coding
		if((idx_list[0] & 0xFFFFC000) == 0) {			// Can put in 14 bits? [2'b10, 14'hXXXX]
			temp1++;
			*((unsigned short *) &compressed_sequence[c_idx]) = (unsigned short) (idx_list[0] | 0x8000);
			c_idx += 2;
		}
		else if((idx_list[0] & 0xFF800000) == 0) {		// Can put in 23 bits? [1'b0, 23'hXXXXXX]
			temp2++;
			*((unsigned int *) &compressed_sequence[c_idx]) = idx_list[0];
			c_idx += 3;
		}
		else {								// Fit in 30 bits. [2'b11, 30'hXXXXXXXX]
			temp3++;
			*((unsigned int *) &compressed_sequence[c_idx]) = idx_list[0] | 0xC0000000;
			c_idx += 4;
		}
	}
...
void treenode::write_details_delta_huffman(std::vector<unsigned int> &idx_list) {
