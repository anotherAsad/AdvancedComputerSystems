	...
	for(int i=0; i<PING_PONG_COUNT; i = (i+1)%PING_PONG_COUNT) {
		...
		if(ping_pong_arr[i].ownership.try_lock()) {
			...
			while(!ping_pong_arr[i].is_empty()) {
				mother_node->addElement(ping_pong_arr[i].pop_elem(), ping_pong_arr[i].base_idx++);
			}
...
int main() {
	...
	for(int i=0; i<thread_count; i++) {
		threadArr[i] = new std::thread(AddElemsFromPingPongs, &mother_node);
	}

	FillPingPongs(&uncompressed_stream);

	std::cout << "\tDictionary Encoded. Will Free up the threads" << std::endl;
